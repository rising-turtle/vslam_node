/*
 *  Aug. 8 2017, He Zhang, hxzhang1@ualr.edu
 *
 *  msckf + svo, loosely couple without dpt info
...
void msckf_node_dpt()
{
  ...
  ros::NodeHandle np("~"); 
  string data_dir = "/home/davidz/work/data/ETAS_F4_640_30"; 
  ...
  for(int i=0; i<mvRgb.size() && ros::ok(); i++)
  {
    ...
    {
      if(vo_->stage() != FrameHandlerBase::STAGE_DEFAULT_FRAME && vo_->stage() != FrameHandlerBase::STAGE_RELOCALIZING)
      {
